...
def generate_candidates_target_addition(adj_matrix, targets):
    ...
    #for item in itertools.product(targets, range(0,num_nodes)):
...
def evaluate_embedding_node_classification(embedding_matrix, labels, targets,train_ratio=0.1, norm=True, seed=0, n_repeats=10):
    ...
    for it_seed in range(n_repeats):
        ...
        features_target = embedding_matrix[targets]
        ...
        lr_z_predict = lr.predict(features_target)
        f1_micro = f1_score(labels_target, lr_z_predict, average='micro')
        f1_macro = f1_score(labels_target, lr_z_predict, average='macro')
