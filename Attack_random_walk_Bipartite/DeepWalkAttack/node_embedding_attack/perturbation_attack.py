...
#@mem.cache
def target_perturbation_top_flips(adj_matrix, k,candidates, n_flips, dim,targets,labels,save_dir=None):
    ...
    embedding_target_new = estimate_target_embeding(vals_new[:,:dim], vecs_new,n_candidates,k+targets)
    lr,candidate_margin_mean=classifier(embedding_org[k:],embedding_target_new,labels,targets,n_candidates)
...
def classifier(embedding_org,embedding_target_new,labels,targets, n_candidates,seed=0, n_repeats=10):
    ...
    for it_seed in range(n_repeats):
        ...
        candidate_margin = lr.predict_proba(embedding_target_new_normalized)[:, 0].reshape(n_candidates, len(
            targets))  # For 2 classes classification!!!!
        margins.append(candidate_margin.mean(1))
    margins=np.array(margins)
    return lr,margins.mean(0)
